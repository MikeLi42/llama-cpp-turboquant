...
// NextN draft head graph (second context). GGUF arch string: qwen35_mtp (Qwen3.6).
llm_build_qwen35_nextn::llm_build_qwen35_nextn(
        const llama_model & model,
        const llm_graph_params & params) :
        llm_graph_context(params) {
    GGML_ASSERT(hparams.nextn_predict_layers > 0 && "QWEN35_NEXTN requires nextn_predict_layers > 0");
    GGML_ASSERT(hparams.nextn_predict_layers == 1 && "QWEN35_NEXTN currently only supports a single NextN block");
    ...
    // qwen35 target arch uses hybrid memory (attn + GDN recurrent). When the draft
    ...
    ggml_tensor * head_norm_w = layer.nextn.shared_head_norm
            ...
            : model.output_norm;
    GGML_ASSERT(head_norm_w && "QWEN35_NEXTN: missing both nextn.shared_head_norm and output_norm");
    cur = build_norm(cur, head_norm_w, nullptr, LLM_NORM_RMS, -1);
    cb(cur, "nextn_shared_head_norm", -1);

    ggml_tensor * head_w = layer.nextn.shared_head_head ? layer.nextn.shared_head_head : model.output;
    GGML_ASSERT(head_w && "QWEN35_NEXTN: missing LM head (nextn.shared_head_head or model.output)");
